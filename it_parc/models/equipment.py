...
class ItParcEquipment(models.Model):
    ...
    def export_inventory_xlsx(self):
        ...
        for row, equip in enumerate(equipments, start=1):
            ...
            worksheet.write(row, 2, equip.model or '')
            worksheet.write(row, 3, dict(equip._fields['category'].selection).get(equip.category, ''))
            worksheet.write(row, 4, equip.site or '')
            worksheet.write(row, 5, equip.employee_id.name or '')
            worksheet.write(row, 6, equip.department_id.name or '')
            worksheet.write(row, 7, equip.contract_id.name or '')
            worksheet.write(row, 8, str(equip.purchase_date or ''))
            worksheet.write(row, 9, float(equip.purchase_value or 0.0))
            worksheet.write(row, 10, str(equip.warranty_end_date or ''))
            worksheet.write(row, 11, dict(equip._fields['state'].selection).get(equip.state, ''))
        workbook.close()
        output.seek(0)
